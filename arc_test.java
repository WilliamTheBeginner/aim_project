...
public class cpt_test
{
  ...
  public static boolean clickCircle(Circle circle, Console con)
  {
	  ...
	  while(true)
	  {
		  ...
		  {
			  if(((Math.pow((con.currentMouseX() - radX), 2) + Math.pow((con.currentMouseY() - radY), 2)) < Math.pow(75, 2)))
			  {
				  con.println(foo);
			  }
		  }
...
class Circle
{
  ...
  {
	  this.radius = 75;
	  this.radX = (int)(Math.random() * 1770);
	  this.radY = (int)(Math.random() * 930);
  }
